...
import { Printer, Room, Device } from 'cranix-common/dist/models/data-model';
...
export class AddPrinterComponent implements OnInit {
  ...
  initValues(id: number) {
    ...
    if (id) {
      ...
      this.ipAdresses = [];
      this.printerNames = {};
      this.roomService.getAvailiableIPs(id).subscribe(
        (val) => {
          for (let a of val) {
            let b = a.split(" ");
  ...
  roomChanged(room: Room) {
    this.printer.roomId = room.id;
    this.initValues(room.id);
  }
  handleFileInput(ev) {
